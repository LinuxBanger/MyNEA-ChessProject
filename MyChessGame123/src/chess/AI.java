...
public class AI {
	...
	public AI(Board board, String colour, Movement movment) throws Exception {
	...
	/**
	 * @return value of pieces. 900 for a king, 90 for a queen, 50 for a rook, 30
	 *         for a bishop, 30 for a knight, 10 for a pawn. If it's none of these,
	 *         returns 0.
	 */
	...
	/**
	 * Evaluates the passed board based on the material (piece) present. (Doesn't
	 * take into account useful positions like castling.)
	 * 
	...
	/**
	 * Calls getNextMove() to find the best next move, and Plays the next move, by
	...
	/**
	 * This tries to maximise (as positive as possible) the board value for itself
	 * (AI colour) and minimise (i.e. as negative as possible) for the other player.
	 * i.e. it finds best move for itself.
	 * 
	 * Adds a value of 50 for castling move. Hence, AI tries to castle, and stop the
	 * human from castling.
	 * 
	 ...
	 */
	private int minimax(int depth, Board board, String playerColour, int alpha, int beta) {
		if (depth == 0) {
		...
		// if it's playing for itself, it will maximize board value (heuristic).
			...
			for (Piece ownPiece : ownPieces) {
				...
				for (Cell dest : moves) {
					...
					// if(move == null)
					// move = tempMove;
					int value = minimax(depth - 1, board, Board.opposite(playerColour), alpha, beta);
	...
	/**
	 * Prints the number of seconds AI took to find the move, and number of nodes of
	 * the game tree visited to find the move.
	 * 
