...
public class GraphicsHandler extends JPanel {
	...
	public GraphicsHandler(Board b, int x, int y, int rowLen, int colLen, int border) {
		...
		Color highlight = new Color(red, green, blue, alpha);
		...
		Color nextmove = new Color(red1, green1, blue1, alpha1);
	...
	public void paintComponent(Graphics graphics) {
		...
		for (int i = 0; i <= (Board.rowMax - Board.rowMin); i++) {
			...
			for (int j = 0; j <= (Board.colMax - Board.colMin); j++) {
				graphics.setColor(Color.decode("#E8EDF9"));
				if (i == 0) {
				...
				if (thisCell.isSelected()) {
					...
					int ovalX = x + border + (colLen - border * 2) / 4;
					int ovalY = y + border + (rowLen - border * 2) / 4;
					int ovalWidth = (colLen - border * 2) / 2;
					int ovalHeight = (rowLen - border * 2) / 2;
	...
	/**
	 * Shows a message in a dialog box notifying that given player has lost the
	 * game. e.g. "White, you lost the game :'( "
	 */
	...
	/**
	 * Sets the gameMode to 1 if it's player vs AI. Sets it to two if it's between
	 * two human players.
	 * 
	...
	/**
	 * Sets AI. If AI is to play next, it lets AI play.
	 * 
	...
	public void setAI(AI ai) throws Exception {
		...
		this.ai = ai;
		if (ai.getColour().equals(board.getCurrentTurn()) && !board.isCheckMate(ai.getColour())
				&& !board.isCheckMate(Board.opposite(ai.getColour())))
			ai.playNextMove();
	}

	/**
	 * Shows a message in a dialog box warning the given player that it is under
	 * check. e.g. "White, you are given a check! :O"
	 */
	...
	/**
	 * Finds the board cell at (x,y) coordinates of graphics. Calls board.clicked()
	 * with the row and column of the clicked cell. Redraws all the graphics of the
	 * window. Checks whether the game has ended, and shows a message if that is
	 * true. if either of the player is under check, shows a message.
	 */
	...
	/**
	 * MouseHandler class which calls clicked() of GraphicsHandler object, with the
