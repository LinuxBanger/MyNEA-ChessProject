...
public class Main {
	...
	/**
	 * Creates welcomeWindow, gameLoaderWindow, gameModeWindow, a new board,
	 * chessWindow, and graphics handler object.
	 * 
	...
	/**
	 * This method is relevant only in 1 player mode. Sets the human colour to given
	 * colour. Creates an AI with the opposite colour.
	 */
	...
	/**
	 * Finds the next window after this window in the window list, Sets this window
	 * as invisible, and next window as visible.
	 */
	...
	/**
	 * Informs about the game mode to graphics handler class. Sets the game's
	 * windows' sequence.
	 * 
	 * If game is in single player mode, the sequence will be as follows: player
	 * selection window, human colour selection window, main chess window.
	 * 
	 * If game is in multiplayer mode, the sequence will be as follows: both player
	 * selection window, main chess window.
	 * 
	 * Sets the first of the windows to visible. All the windows call nextWindow
	 * method to enable next window.
	 */
	public void setWindowList() {
		try {
			...
			e.printStackTrace();
			System.out.println("Exception in setWindowList() of Main" + "while setting game mode.");
		}

		if (gameMode == 1) {
			selectPlayerWindow = new SelectPlayerWindow(1);
			selectPlayerWindow.setSize(300, 300);
			selectPlayerWindow.setLocationRelativeTo(null);
	...
	/**
	 * On closing of the chessWindow, updates the statistics of the player(s), and
	 * saves game statistics. (If game is new, creates new game object. Calls
	 * storeMoves() and storeGame() of the game object.)
	 * 
	 * Displays a toolbar containing undo button. On clicking undo button, undoes
	 * last opponent move and last own move, And, lets the current player play
	 * again.
	 * 
	...
	private class ChessWindow extends JFrame {
		...
		private ImageIcon getImage(int height, int width, String path) {
			...
			try {
				...
				e.printStackTrace();
				System.out.println("Exception while getting " + path + " icon" + " in ChessWindow class.");
			}
			...
			public void windowClosing(WindowEvent e) {
				if (Main.this.game == null) {
					try {
						...
						game = new Game(gameMode, player.toString(), player2.toString(), colour, colour2,
	...
	/**
	 * This window has relevance only when game mode is 1 player. Lets the user
	 * select his/her colour. Sets the AI colour to opposite of the player colour.
	 */
	private class SelectColourWindow extends JFrame {
		...
		/**
		 * Closes the current window, and calls nextWindow() from main, to display the
		...
		/**
		 * As soon as user selects the colour, set the colour for it, and close this
		 * window.
		 */
	...
	/**
	 * Displays a window to let user(s) select their name(s) from the saved players'
	 * statistics file.
	 * 
	 * Also supports creation of a new Player (with a new name). (Two players can't
	 * have same name.)
	 * 
	 * If game is single player, shows only one drop down menu for name. Otherwise
	 * shows two drop-down menus for selecting names.
	 * 
	...
	private class SelectPlayerWindow extends JFrame {
		...
		/**
		 * If user enters an already existing name, it loads the player object from the
		 * stored file. Otherwise creates a new Player with given (typed) name.
		 * 
		 * Validates whether two names are equal in 2 player mode.
		 * 
		 * If both names are valid, creates/sets the required players, and closes the
		 * window.
		 */
	...
	/**
	 * Displays a window asking user for game mode, and sets the game mode. The game
	 * has two modes currently : single player and multiplayer. After setting the
	 * game mode successfully, this class calls the setWindowList() method of the
	 * main class, which continues the flow.
	 * 
	...
	private class GameModeWindow extends JFrame {
		...
		/**
		 * Sets the game mode according to the clicked button. On successful set,
		 * immediately closes the frame.
		 */
	...
	/**
	 * Welcomes the user and gives a choice to either start a new game or load a
	 * previous game.
	 */
	private class WelcomeWindow extends JFrame {
		...
		/**
		 * If player wants a new game, call game mode window. Otherwise call game loader
		 * window.
		 */
		private class HandlerClass implements ItemListener {
			@Override
	...
	/**
	 * If user chooses to load a previous game, displays all the previously stored
	 * games. (Sorted by time of creating the game object.)
	 */
	private class GameLoaderWindow extends JFrame {
		...
		/**
		 * Closes the current window, reloads the game, makes chessWindow visible, and
		 * sets game mode to whatever it was. If game was one player, sets human colour
		...
		public void CloseFrame() {
			...
			Main.this.chessWindow.setLocationRelativeTo(null);
			try {
		...
		/**
		 * If user selects a game, it loads the game object from the stored file.
		 */
		private class HandlerClass implements ActionListener {
			public void actionPerformed(ActionEvent event) {
				if (Main.this.game != null)
